...
function deleteItem(index) {
    ...
    const list = document.getElementById('custom-replies-list');
    if (list) {
        const cards = list.querySelectorAll('.rl-card');
        if (cards[index]) {
            ...
            setTimeout(() => {
                cards[index].remove();
                const remaining = list.querySelectorAll('.rl-card');
                remaining.forEach((card, i) => {
                    const delBtn = card.querySelector('[data-action="delete"]');
                    const editBtn = card.querySelector('[data-action="edit"]');
                    if (delBtn) delBtn.onclick = (e) => { e.stopPropagation(); deleteItem(i); };
                    if (editBtn) editBtn.onclick = (e) => { e.stopPropagation(); editItem(i, ctx.items[i]); };
                });
            }, 200);
        } else {
            renderReplyLibrary();
