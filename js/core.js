...
autoSendInterval: 5,
        ...
        stitchReplyMin: 2,
        stitchReplyMax: 4,
        pureStickerReply: false,
        // 背景设置
                bgScope: 'chat+header',
                bgDisplayMode: 'cover',
                bgOpacity: 80,
                bgBlur: 0,
                bgBubbleEnhance: true
            };
        }
        ...
        const applyBackground = (value) => {
            ...
            try {
                ...
                if (value.startsWith('linear-gradient') || value.startsWith('#') || value.startsWith('rgb')) {
                    ...
                    cssImageValue = value.startsWith('url(') ? value : `url("${value}")`;
                }
                document.documentElement.style.setProperty('--chat-bg-image', cssImageValue);
                
                const scope = settings.bgScope || 'chat+header';
                document.documentElement.style.setProperty('--bg-scope', scope);
                
                const mode = settings.bgDisplayMode || 'cover';
                let bgSize = 'cover', bgRepeat = 'no-repeat', bgPosition = 'center center';
                if (mode === 'stretch') { bgSize = '100% 100%'; }
                else if (mode === 'repeat') { bgSize = 'auto'; bgRepeat = 'repeat'; }
                else if (mode === 'center') { bgSize = 'auto'; }
                document.documentElement.style.setProperty('--bg-size', bgSize);
                document.documentElement.style.setProperty('--bg-repeat', bgRepeat);
                document.documentElement.style.setProperty('--bg-position', bgPosition);
                
                const opacity = (settings.bgOpacity || 80) / 100;
                const blur = (settings.bgBlur || 0) + 'px';
                document.documentElement.style.setProperty('--bg-opacity', opacity);
                document.documentElement.style.setProperty('--bg-blur', blur);
                
                if (settings.bgBubbleEnhance) {
...
const loadData = async () => {
    try {
        ...
        // 背景设置默认值保护
        if (settings.bgScope === undefined) settings.bgScope = 'chat+header';
        if (settings.bgDisplayMode === undefined) settings.bgDisplayMode = 'cover';
        if (settings.bgOpacity === undefined) settings.bgOpacity = 80;
        if (settings.bgBlur === undefined) settings.bgBlur = 0;
        if (settings.bgBubbleEnhance === undefined) settings.bgBubbleEnhance = true;
...
window.simulateReply = function() {
       // ✅ 梦角发红包
    if (settings.redPacketEnabled && (settings.partnerBalance || 0) > 0) {
        const isSpecial = (() => {
            const today = new Date();
            const mm = String(today.getMonth() + 1).padStart(2, '0');
            const dd = String(today.getDate()).padStart(2, '0');
            return (settings.specialDateList || []).includes(mm + '-' + dd);
        })();
        const prob = isSpecial ? 0.8 : 0.05;
        if (Math.random() < prob) {
            const useSpecial = isSpecial ? (Math.random() < 0.9) : (Math.random() < 0.4);
            let amount;
            if (useSpecial) {
                const specialNumbers = [5.20, 13.14, 52.0, 520, 1314, 5200];
                amount = specialNumbers[Math.floor(Math.random() * specialNumbers.length)];
            } else if (Math.random() < 0.2) {
                amount = settings.partnerBalance || 0;
            } else {
                amount = Math.floor(Math.random() * 200) + 1;
            }
            if (amount > 0 && amount <= (settings.partnerBalance || 0)) {
                sendRedPacket('partner', amount);
                return;
            }
        }
    }
  // ✅ 纯贴纸回复模式
    if (settings.pureStickerReply && stickerLibrary && stickerLibrary.length > 0) {
        const disabledStickerItems = (() => {
            try {
                const raw = localStorage.getItem('disabledStickerItems');
                return raw ? new Set(JSON.parse(raw)) : new Set();
            } catch (e) { return new Set(); }
        })();
        const enabledStickers = stickerLibrary.filter(s => !disabledStickerItems.has(s));
    ...
    for (let i = 0; i < replyCount; i++) {
        ...
        setTimeout(() => {
            try {
                ...
                try {
                    (function(){ try { if (window._typingIndicatorAutoHideTimer) { clearTimeout(window._typingIndicatorAutoHideTimer); window._typingIndicatorAutoHideTimer = null; } } catch (e2) {} var _tiW2 = document.getElementById('typing-indicator-wrapper'); if (_tiW2) _tiW2.style.display = 'none'; })();
                } catch (e2) {}
            }
        }, delay);
    }
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (msg.type === 'call-event') {
        ...
        const packet = msg.redPacket;
        const claimed = packet ? packet.claimed : true;
        ...
        const sendTime = getTimeAgo(packet ? packet.timestamp : null);
        const claimTime = getTimeAgo(packet ? packet.claimedAt : null);

        rpDiv.innerHTML = `
            <div class="message-content-wrapper">
                <div class="red-packet-card" data-packet-id="${packet ? packet.id : ''}" style="
                    background: ${claimed ? '#e8e8e8' : '#8b1a1a'};
                    border-radius: 8px;
                    padding: 0;
                    ...
                    text-align: center;
                    transition: all 0.2s;
                    box-shadow: ${claimed ? '0 2px 6px rgba(0,0,0,0.08)' : '0 4px 12px rgba(139,26,26,0.4)'};
                    overflow: hidden;
                ">
                    <!-- 上半部分：暗红背景 -->
                    <div style="
                        background: ${claimed ? '#e0e0e0' : '#8b1a1a'};
                    ...
                    ">
                        <div style="flex-shrink:0;">
                            <svg width="32" height="38" viewBox="0 0 32 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <rect x="2" y="8" width="28" height="28" rx="4" stroke="white" stroke-width="2" fill="none"/>
                                <path d="M2 18 L16 28 L30 18" stroke="white" stroke-width="2" fill="none" stroke-linecap="round"/>
                                <circle cx="16" cy="22" r="5" stroke="white" stroke-width="1.5" fill="none"/>
                            </svg>
                        </div>
                        <div style="
                            font-size:20px;
                            font-weight:700;
                            color:${claimed ? '#999' : '#fff'};
                            font-family:'PingFang SC','Microsoft YaHei','Noto Sans SC',sans-serif;
                            text-align:right;
                        ">¥${packet ? packet.amount : '?'}</div>
                    </div>
                    <!-- 金色虚线分隔 -->
                    <div style="
                        height:2px;
                        background-image:repeating-linear-gradient(90deg, ${claimed ? '#bbb' : '#d4a843'} 0, ${claimed ? '#bbb' : '#d4a843'} 6px, transparent 6px, transparent 12px);
                    "></div>
                    <!-- 下半部分：白色背景 -->
                    <div style="
                        background: ${claimed ? '#f5f5f5' : '#fff'};
                    ...
                    ">
                        ...
                        <div style="
                            ...
                            color:#999;
                            text-align:right;
                        ">${claimed ? claimTime : sendTime}</div>
                    </div>
                </div>
            </div>`;

        if (!claimed && packet && packet.sender !== 'user') {
            rpDiv.addEventListener('click', function() {
                claimRedPacket(packet.id, 'user');
            });
        }
...
function renderMessages(preserveScroll = false) {
    ...
    container.appendChild(fragment);
       ...
       // 刷新红包卡片状态
    container.querySelectorAll('.red-packet-card').forEach(card => {
        const pid = card.dataset.packetId;
        const packet = (settings.redPacketHistory || []).find(p => p.id === pid);
        if (packet && packet.claimed && !card.classList.contains('claimed')) {
            card.style.background = '#e0e0e0';
            card.querySelectorAll('div').forEach(d => d.style.color = '#999');
            card.style.boxShadow = 'none';
            card.style.cursor = 'default';
            const statusDiv = card.querySelector('div:last-child');
            if (statusDiv) statusDiv.textContent = '已领取';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function sendRedPacket(sender, amount) {
    if (amount <= 0) return null;
    if (sender === 'user' && amount > (settings.myBalance || 0)) {
        showNotification('余额不足', 'error');
        return null;
    }
    if (sender === 'partner' && amount > (settings.partnerBalance || 0)) return null;

    if (sender === 'user') settings.myBalance = Math.max(0, (settings.myBalance || 0) - amount);
    if (sender === 'partner') settings.partnerBalance = Math.max(0, (settings.partnerBalance || 0) - amount);
    ...
    if (!settings.redPacketHistory) settings.redPacketHistory = [];
    settings.redPacketHistory.unshift(packet);

    addMessage({
        id: Date.now(),
        sender: sender === 'user' ? 'user' : (settings.partnerName || '对方'),
        text: '',
        timestamp: new Date(),
        type: 'red-packet',
        redPacket: packet,
    ...
    if (sender === 'user') {
        const claimDelay = 2000 + Math.random() * 18000;
        setTimeout(() => claimRedPacket(packet.id, 'partner'), claimDelay);
    }
...
function claimRedPacket(packetId, claimer) {
    const packet = (settings.redPacketHistory || []).find(p => p.id === packetId);
    if (!packet || packet.claimed || packet.expired) return;

    packet.claimed = true;
    packet.claimedAt = Date.now();

    if (claimer === 'user') {
        settings.myBalance = (settings.myBalance || 0) + packet.amount;
    } else {
        settings.partnerBalance = (settings.partnerBalance || 0) + packet.amount;
    }
...
function showRedPacketHistory() {
    const history = settings.redPacketHistory || [];
    const overlay = document.createElement('div');
    ...
    let listHTML = '';
    if (history.length === 0) {
        listHTML = '<div style="text-align:center;padding:40px;color:var(--text-secondary);">暂无红包记录</div>';
    } else {
        listHTML = history.map((p) => {
            const date = new Date(p.timestamp);
            const timeStr = date.toLocaleDateString('zh-CN') + ' ' + date.toLocaleTimeString('zh-CN', {hour:'2-digit',minute:'2-digit'});
            const sender = p.sender === 'user' ? (settings.myName || '我') : (settings.partnerName || '对方');
            const status = p.claimed ? '已领取' : (p.expired ? '已过期' : '待领取');
            const statusColor = p.claimed ? '#51cf66' : (p.expired ? '#999' : '#f96b3a');
            return `
                <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-bottom:1px solid var(--border-color);">
                    <div>
                        <div style="font-size:13px;color:var(--text-primary);">${sender} 发的红包</div>
                        <div style="font-size:11px;color:var(--text-secondary);">${timeStr}</div>
                    </div>
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="font-size:14px;font-weight:600;color:var(--text-primary);">¥${p.amount}</span>
                        <span style="font-size:11px;color:${statusColor};">${status}</span>
                        <button onclick="window._deleteRpRecord('${p.id}')" style="background:none;border:none;color:#ef4444;cursor:pointer;font-size:12px;">🗑</button>
                    </div>
                </div>`;
        }).join('');
    }
    ...
    overlay.innerHTML = `
        <div style="background:var(--secondary-bg);border-radius:20px;padding:20px;width:92%;max-width:400px;max-height:80vh;display:flex;flex-direction:column;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                <div style="font-size:16px;font-weight:700;color:var(--text-primary);">🧧 红包记录</div>
                <button id="_rp_hist_close" style="width:30px;height:30px;border-radius:50%;border:none;background:var(--primary-bg);color:var(--text-secondary);cursor:pointer;font-size:16px;">×</button>
            </div>
            <div style="overflow-y:auto;flex:1;">${listHTML}</div>
        </div>`;
    document.body.appendChild(overlay);

    overlay.querySelector('#_rp_hist_close').onclick = () => overlay.remove();
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
}

window._deleteRpRecord = function(id) {
    if (!confirm('删除这条红包记录？')) return;
    settings.redPacketHistory = (settings.redPacketHistory || []).filter(p => p.id !== id);
    throttledSaveData();
...
setInterval(() => {
    const now = Date.now();
    let changed = false;
    (settings.redPacketHistory || []).forEach(p => {
        if (!p.claimed && !p.expired && (now - p.timestamp > 24 * 60 * 60 * 1000)) {
            p.expired = true;
            if (p.sender === 'user') settings.myBalance = (settings.myBalance || 0) + p.amount;
            if (p.sender === 'partner') settings.partnerBalance = (settings.partnerBalance || 0) + p.amount;
            changed = true;
        }
    });
    if (changed) {
...
window.showRedPacketHistory = showRedPacketHistory;
});